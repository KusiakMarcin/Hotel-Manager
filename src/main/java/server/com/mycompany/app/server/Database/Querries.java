...
public class Querries {
    ...
    public Map<String,String> Personel = new HashMap<>();
    ...
    public Querries(){
        ...
        Booking.put("GET_GUESTS","SELECT * FROM GUESTS WHERE RESERVATION_RESERVATION_ID = ?");
        Booking.put("GET_TABLE","SELECT * FROM RESERVATION");
        Booking.put("GET_SERVICES","SELECT * FROM RESERVATION_SERVICE WHERE RESERVATION_ID = ? ");
        Booking.put("GET_PAYMENTS","SELECT * FROM RESERVATION_PAYMENTS WHERE RESERVATION_ID = ?");

        Guest.put("ADD_GUEST","INSERT INTO GUESTS (GUEST_ID,CLIENT_CLIENT_ID)");
        Guest.put("UPDATE","UPDATE SET # = ? WHERE GUEST_ID = ?");
        Guest.put("DELETE","DELETE FROM GEUSTS WHERE GUEST_ID = ?");
        Guest.put("GET_TABLE","SELECT * FROM GUESTS");

        Personel.put("INSERT","INSERT INTO PERSONEL (employee_id,employee_name,employee_last_name,employee_nationality,employee_birth_date,employee_position)VALUES(personel_id_incr.nextval,?,?,?,?,?");
